...
public struct SessionConfiguration: Codable, Equatable, Sendable {
    ...
    private enum CodingKeys: String, CodingKey { case harness, model, reasoning, interactionMode }
...
public struct Approval: Codable, Equatable, Sendable, Identifiable {
    ...
    public init(id: UUID = UUID(), title: String, detail: String) { self.id = id; self.title = title; self.detail = detail }
}
...
public enum SessionPhase: Codable, Equatable, Sendable {
    ...
    public var approval: Approval? { if case .needsApproval(let approval) = self { return approval }; return nil }
...
public struct ChatMessage: Codable, Equatable, Sendable, Identifiable {
    ...
    public init(id: UUID = UUID(), role: Role, text: String, steps: [String] = []) { self.id = id; self.role = role; self.text = text; self.steps = steps }
}
...
public struct DiffLine: Codable, Equatable, Sendable {
    ...
    public var prefix: String { switch kind { case .context: " "; case .addition: "+"; case .deletion: "−" } }
}
...
public struct FileChange: Codable, Equatable, Sendable, Identifiable {
    ...
    public init(path: String, lines: [DiffLine]) { self.path = path; self.lines = lines }
}
...
public struct AgentSession: Codable, Equatable, Sendable, Identifiable {
    ...
    public init(appSessionId: UUID = UUID(), title: String, workspace: String = "droid-maxxing", configuration: SessionConfiguration = .init(), phase: SessionPhase = .ready, messages: [ChatMessage] = [], changes: [FileChange] = [], draft: String = "", updatedAt: Date = .now) {
        self.appSessionId = appSessionId; self.title = title; self.workspace = workspace; self.configuration = configuration; self.phase = phase; self.messages = messages; self.changes = changes; self.draft = draft; self.updatedAt = updatedAt
    }
}
...
public enum AgentEvent: Sendable { case step(String); case text(String); case changes([FileChange]); case approval(Approval); case completed }

@MainActor public protocol AgentClient { func events(for request: TurnRequest) -> AsyncThrowingStream<AgentEvent, Error> }
