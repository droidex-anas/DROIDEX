                ...
                .toolbar {
                    ToolbarItem(placement: .principal) {
                        VStack(spacing: 3) {
                            ...
                            Text("\(session.configuration.harness.rawValue) · Local preview")
                    ...
                    ToolbarItem(placement: .topBarTrailing) {
                        Menu {
                            ...
                            ShareLink(item: exportedConversation(session)) {
                ...
                .safeAreaInset(edge: .bottom, spacing: 0) {
                    ComposerView(
                        ...
                        configuration: Binding(
                            get: { store.session(sessionID)?.configuration ?? .init() },
                            set: { store.configure(sessionID, with: $0) }
                        ),
                        phase: session.phase,
    ...
    private func conversation(_ session: AgentSession) -> some View {
        ScrollViewReader { proxy in
                ...
                LazyVStack(alignment: .leading, spacing: 28) {
                    ...
                    if session.phase.isRunning {
                        HStack(spacing: 10) {
                            ...
                            Text("Running the preview…").font(.footnote)
                        }
                        .foregroundStyle(DroidTheme.secondary)
                        .accessibilityLabel("The preview is working")
                    } else {
                        PhaseLabel(phase: session.phase)
                    }
                    if case .failed(let error) = session.phase {
                        Label(error, systemImage: "exclamationmark.triangle")
                            .font(.callout).foregroundStyle(DroidTheme.danger)
                    }
                    if !session.changes.isEmpty {
                        changesButton(session)
                    }
            ...
            .onChange(of: session.messages.last?.text) { _, _ in
                if isAtBottom { proxy.scrollTo(bottomID, anchor: .bottom) }
            }
    ...
    private func changesButton(_ session: AgentSession) -> some View {
        Button { showReview = true } label: {
            VStack(alignment: .leading, spacing: 10) {
                Label("Review changes", systemImage: "doc.text")
            ...
            VStack(alignment: .leading, spacing: 14) {
                ...
                if !message.steps.isEmpty {
                    DisclosureGroup {
                        ...
                        Text("\(message.steps.count) preview steps").font(.footnote)
                    }
                    .foregroundStyle(DroidTheme.secondary)
                }
