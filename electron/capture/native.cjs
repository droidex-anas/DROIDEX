...
function runCapture(args, signal, execute = execFile) {
  return new Promise((resolve, reject) => {
    execute('/usr/sbin/screencapture', args, { signal, timeout: 180_000, maxBuffer: 32_768 }, error => {
...
async function captureNative(mode, signal) {
  if (process.platform !== 'darwin') throw new Error('Desktop snipping currently requires macOS. Image import and editing are available here.');
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), 'droidex-capture-'));
  await fs.chmod(dir, 0o700);
  const destination = path.join(dir, 'capture.png');
  try {
    let captureError;
    try { if (!(await runCapture(captureArguments(mode, destination), signal))) return null; }
    catch (error) { captureError = error; }
    if (signal.aborted) return null;
    const stat = await fs.stat(destination).catch(error => { if (error.code === 'ENOENT') return null; throw error; });
    // macOS returns 1 and creates no file when the user presses Escape.
    if (!stat || stat.size === 0) {
      if (captureError && captureError.code !== 1) throw new Error('Screen capture failed. Check macOS Screen Recording permission and try again.', { cause: captureError });
      return null;
    }
    if (stat.size > MAX_IMAGE_BYTES) throw new Error('Screenshot exceeds the 40 MiB limit');
    const buffer = await fs.readFile(destination); png(buffer); return buffer;
...
async function captureComponent(contents, selection) {
  ...
  const buffer = image.toPNG(); png(buffer); return buffer;
}
