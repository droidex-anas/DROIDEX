...
test('concurrent editors cannot overwrite a newer revision or use a thumbnail as export', async t => {
  ...
  await assert.rejects(store.save(record.id, 1, { ...recipe, style: { ...bare, padding: 30 } }, PNG, PNG), /dimensions/);
...
test('serialized concurrent captures keep the bounded history within its count limit', async t => {
  const { store } = await fixture(t);
  await Promise.all(Array.from({ length: MAX_RECORDS + 4 }, (_, i) => store.create(PNG, `Capture ${i}`)));
...
test('capture storage rejects symlink roots and symlinked originals', async t => {
  ...
  const target = path.join(dir, 'other'); await fs.mkdir(target);
  const linked = path.join(dir, 'linked'); await fs.symlink(target, linked);
  ...
  await fs.rm(source); await fs.writeFile(path.join(target, 'outside.png'), PNG); await fs.symlink(path.join(target, 'outside.png'), source);
...
test('capture IPC rejects subframes, embedded browsers and absent windows', () => {
  const frame = {}; const contents = { mainFrame: frame };
...
test('native capture uses argument arrays, suppresses native sound, and does not write clipboard', () => {
  assert.deepEqual(captureArguments('area', '/tmp/capture.png'), ['-x', '-i', '-s', '-t', 'png', '/tmp/capture.png']);
...
test('native cancellation resolves without a result and failure stays observable', async () => {
  ...
  const cancelled = await runCapture([], controller.signal, (command, args, options, callback) => {
    assert.equal(command, '/usr/sbin/screencapture'); assert.equal(options.signal, controller.signal); assert.equal(options.timeout, 180000);
    controller.abort(); callback(new Error('Aborted'));
  });
  assert.equal(cancelled, false);
  await assert.rejects(runCapture([], new AbortController().signal, (_command, _args, _options, callback) => callback(new Error('Permission denied'))), /Permission denied/);
});
test('component capture converts CSS bounds through zoom and exports the highest density', async () => {
  ...
  const contents = {
    ...
    getZoomFactor: () => 1.5,
    capturePage: async rect => { box = rect; return { isEmpty: () => false, getScaleFactors: () => [1, 2], toPNG: options => { density = options.scaleFactor; return PNG; } }; },
  };
  ...
  await assert.rejects(captureComponent(contents, { x: 499, y: 0, width: 10, height: 10 }), /outside/);
});
