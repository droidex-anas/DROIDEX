...
contextBridge.exposeInMainWorld('droidCapture', {
  ...
  setPreferences: value => ipcRenderer.invoke('capture:request', { operation: 'setPreferences', value }),
  take: request => ipcRenderer.invoke('capture:request', { ...request, operation: 'take' }),
  cancel: requestId => ipcRenderer.invoke('capture:request', { operation: 'cancel', requestId }),
  import: (source, title) => ipcRenderer.invoke('capture:request', { operation: 'import', source, title }),
  ...
  save: (id, revision, recipe, output) => ipcRenderer.invoke('capture:request', { operation: 'save', id, revision, recipe, output }),
