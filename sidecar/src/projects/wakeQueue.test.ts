...
function project(id = 'project'): Project {
  return {
    id, title: id, paused: false, wakesLeft: 20, launching: 0,
    threads: [
      { appSessionId: 'main', title: 'Main', reply: '', waiting: false },
      { appSessionId: 'worker', ownerAppSessionId: 'main', title: 'Worker', reply: '', waiting: false },
    ],
...
test('acceptance removes only its claim and holds the turn slot until completion', async () => {
  ...
  }, async () => { saved = structuredClone(state); }, (_project, error) => { throw error; });
  queue.kick(state);
  await tick();
  ...
  await tick();
  assert.deepEqual(state.pending.map((item) => item.id), ['arrived-during-admission']);
...
test('availability arriving during an awaited busy receipt is not lost', async () => {
  ...
  }, async () => {}, (_project, error) => { throw error; });
  queue.kick(state);
  await tick();
  queue.available(state, 'main');
...
test('completed callbacks free the global limit of two accepted project turns', async () => {
  ...
  states.forEach((item, index) => { item.pending[0].to = `main-${index}`; });
  const finished = deferred<void>();
  let calls = 0;
  const queue = new ProjectWakeQueue({
    deliver: async () => {
