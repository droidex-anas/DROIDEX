...
export function createProjectMcpServer(
  ...
  const target = z
    ...
    .min(1)
    .max(200)
    .describe('Stable DROIDEX appSessionId returned by thread_spawn/list.');
  ...
  return createSdkMcpServer({
    ...
    tools: [
      ...
      tool(
        'thread_list',
        'List this project’s threads and available harness/model choices. Does not wake any agent. Never poll this tool while waiting.',
        {},
        safeTool(async () =>
          jsonResult({
            ...(await projects()).inspect(source()),
            providers: catalog().map(({ provider, readiness, models, defaultModelId }) => ({
              provider,
              readiness,
              models,
              defaultModelId,
      ...
      tool(
        'thread_read',
        'Read a project thread’s latest bounded reply and current settings/status without waking it. Full transcripts remain in the normal thread UI.',
        { appSessionId: target },
        safeTool(async ({ appSessionId }) =>
          jsonResult((await projects()).inspect(source(), appSessionId)),
        ),
      ),
      tool(
        'thread_send',
        'Queue instructions or an answer for a thread you own. The main project thread may address any member. Delivery waits until the recipient is idle and never interrupts a user turn.',
        { appSessionId: target, text },
      ...
      tool(
        'thread_ask',
        'Ask your owning thread a coordination question, then finish your turn. This returns immediately; the answer arrives in a later turn. Use native user questions/approvals for user intent or permission, never this tool.',
        { question: text },
        safeTool(async ({ question }) => {
          await (await projects()).ask(source(), question);
          return jsonResult({
            queued: true,
            next: 'Finish your turn while waiting for the answer.',
          });
        }),
      ),
      tool(
        'thread_stop',
        'Interrupt a thread you own and cancel its queued project messages. Keeps its conversation and history.',
        { appSessionId: target },
        safeTool(async ({ appSessionId }) => {
