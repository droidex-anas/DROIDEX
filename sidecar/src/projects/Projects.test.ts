...
import { Projects, type ProjectSessions } from './Projects.js';
import type { ProjectPersistence } from './projectStore.js';
import type { Project, ThreadInput } from './types.js';
import type { ServerEvent, SessionSummary } from '../protocol.js';

const input: ThreadInput = { title: 'Build', prompt: 'Build the feature.', provider: 'droid', autonomy: 'low', cwd: '/workspace' };
const tick = () => new Promise<void>((resolve) => setImmediate(resolve));
async function drain() { for (let i = 0; i < 8; i += 1) await tick(); }
function deferred() {
  let resolve = () => {};
...
function summary(id: string, selection: ThreadInput = input): SessionSummary {
  return {
    appSessionId: id, providerSessionId: id, provider: selection.provider,
    sessionPurpose: 'chat', interactionMode: 'auto', role: 'user', title: selection.title,
    goal: selection.prompt, cwd: selection.cwd ?? '', modelId: selection.modelId,
    reasoningEffort: selection.reasoningEffort, autonomy: selection.autonomy,
    phase: 'running', streaming: false, features: [], tokensIn: 0, tokensOut: 0,
    contextTokens: 0, createdAt: 1, updatedAt: 1,
  };
}
...
async function harness(saved: Project[] = []) {
  ...
  const state = { saved: structuredClone(saved), failSave: false, gate: undefined as Promise<void> | undefined, bindGate: undefined as Promise<void> | undefined };
  ...
  const port: ProjectSessions = {
    ...
    sendWhenIdle: async (id, prompt, isCurrent) => {
      if (state.gate) await state.gate;
      const session = sessions.get(id);
      if (!isCurrent() || !session || session.streaming) return false;
      sent.push({ id, prompt });
      await streaming(id, true);
      return true;
    },
    interrupt: async (id) => { await streaming(id, false); },
  };
  projects = await Projects.open(port, store, (event) => events.push(event));
  ...
  async function finish(id: string, text = 'Done') {
    await projects.observe({ type: 'event.appended', event: {
      id: `${id}-text`, appSessionId: id, sourceSessionId: id, role: 'primary', ts: 1, kind: 'text', text,
    } });
  ...
  return { projects, sessions, sent, launched, events, state, store, port, streaming, finish, root };
}
...
test('idle projects produce no turns; one completed child wakes its owner once', async (t) => {
  ...
  await drain(); assert.equal(h.sent.length, 0);
  const child = await h.projects.spawn(main, { ...input, provider: 'codex', modelId: 'code', reasoningEffort: 'high' });
  await drain(); assert.equal(h.sent.length, 0);
  ...
  await drain(); assert.equal(h.sent.length, 1);
...
test('ordinary chats adopt a project, with scoped ownership and no autonomy escalation', async (t) => {
  ...
  assert.throws(() => h.projects.inspect('ordinary', other.main), /outside/);
  ...
  await assert.rejects(h.projects.spawn(child.appSessionId, { ...input, autonomy: 'high' }), /autonomy/);
...
test('pause cancels a pending wake after asynchronous admission work', async (t) => {
  ...
  const child = await h.projects.spawn(main, input);
  const gate = deferred(); h.state.gate = gate.promise;
...
test('stop waits for a cancelled claim before removing target messages', async (t) => {
  ...
  gate.resolve(); await stopped; await drain();
  assert.equal(h.sent.some((item) => item.id === child.appSessionId), false);
  assert.equal(h.state.saved[0]?.pending.some((item) => item.to === child.appSessionId), false);
});

test('parallel spawn reservations cap fanout before provider creation', async (t) => {
  const h = await harness(); t.after(() => h.projects.close());
  const { id, main } = await h.root();
  const gate = deferred(); h.state.bindGate = gate.promise;
...
test('restart preserves an uncertain delivery and never replays it implicitly', async (t) => {
  ...
  const recovered = await harness(disk); t.after(() => recovered.projects.close());
  ...
  await drain(); assert.equal(recovered.sent.length, 0);
  assert.equal(recovered.projects.list()[0]?.uncertain, 0);
  h.projects.close(); gate.resolve(); await h.projects.flush();
});
...
test('native permissions and user questions never generate controller turns', async (t) => {
  const h = await harness(); t.after(() => h.projects.close());
  const { main } = await h.root();
  await h.projects.observe({ type: 'approval.requested', request: { appSessionId: main, requestId: 'approval', kind: 'exec', title: 'Run?', detail: 'A command', raw: {} } });
  await h.projects.observe({ type: 'question.requested', question: { appSessionId: main, requestId: 'question', questions: [{ index: 0, question: 'User decision?', options: [] }] } });
  await drain(); assert.equal(h.sent.length, 0);
});
