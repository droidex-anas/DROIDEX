...
export const threadInputSchema = z.object({
  ...
  reasoningEffort: z.enum([
    'off', 'none', 'minimal', 'low', 'medium', 'high', 'xhigh', 'max', 'ultra', 'dynamic',
  ]).optional(),
...
export class ProjectStore implements ProjectPersistence {
  ...
  async load(): Promise<Project[]> {
    let raw: string;
    try {
      if ((await stat(this.path)).size > MAX_BYTES) throw new Error('Project ledger exceeds 8 MiB.');
    ...
    for (const item of projects) {
      ...
      if (item.threads.length && item.threads.filter((thread) => !thread.ownerAppSessionId).length !== 1) {
      ...
      for (const note of [...item.pending, ...(item.delivery?.messages ?? [])]) {
        if (!threads.has(note.from) || !threads.has(note.to)) throw new Error('Unknown delivery target.');
      }
    }
    return projects;
  }
  ...
  save(projects: Project[]): Promise<void> {
    ...
    if (Buffer.byteLength(json) > MAX_BYTES) return Promise.reject(new Error('Project ledger exceeds 8 MiB.'));
