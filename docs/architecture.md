...
- App-owned provider transcripts serialize appends through an asynchronous file-write chain. Turn settlement and close await the chain; a failed append rejects the boundary and prevents later lines from overtaking the missing row. No transcript file or extra worker is opened at session construction.
