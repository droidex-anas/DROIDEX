...
export default function StudioModelPicker({
  ...
  return (
    <div className="relative min-w-0" ref={ref}>
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex min-w-0 items-center gap-1.5 rounded-lg border border-droid-border bg-white/[0.03] px-2 py-1.5 text-[11.5px] text-droid-text-secondary transition-colors hover:border-droid-border hover:text-droid-text"
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="absolute bottom-full left-0 z-30 mb-1.5 w-64 overflow-hidden rounded-xl border border-droid-border bg-droid-elevated shadow-2xl"
          >
  ...
  return (
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className={`block truncate text-[12px] ${selected ? 'text-[#f0a060]' : 'text-droid-text'}`}>
          {label}
        </span>
