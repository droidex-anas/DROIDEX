...
export function authoringInstruction(directions = 3, referenceIds: readonly string[] = []): string {
  ...
  return [
    ...
    '2. MOTION.md — easing curves, hover/press/enter behavior, reduced-motion policy, and this exact',
    '   executable block: `motion-tokens` JSON with `durations` (milliseconds; number or [min,max]),',
    '   `easings` (CSS easing strings), optional `pressScale`, and `reducedMotion` (`disable` or `reduce`).',
