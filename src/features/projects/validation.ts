...
export function isProjectView(value: unknown): value is ProjectView {
  ...
  const owners = new Map(value.threads.map((thread) => [thread.appSessionId, thread.ownerAppSessionId]));
...
function isProjectMetadata(value: Record<string, unknown>): boolean {
  return text(value.id, 200) && text(value.title, 120) && typeof value.paused === 'boolean'
    && count(value.wakesLeft, 20) && count(value.launching, 8)
    && count(value.queued, 64) && count(value.uncertain, 64)
    && (value.error === undefined || text(value.error, 2_000));
}

function isThreadList(value: unknown): value is ProjectThread[] {
  return Array.isArray(value) && value.length <= 8 && value.every((thread: unknown) =>
    record(thread) && text(thread.appSessionId, 200) && text(thread.title, 120)
