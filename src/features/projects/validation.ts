...
export function isProjectView(value: unknown): value is ProjectView {
  ...
  if (typeof value.paused !== 'boolean' || !count(value.wakesLeft, 20) || !count(value.launching, 8)) return false;
  ...
  for (const thread of value.threads) {
    if (!record(thread) || !text(thread.appSessionId, 200) || !text(thread.title, 120) || typeof thread.waiting !== 'boolean') return false;
    if (thread.ownerAppSessionId !== undefined && !text(thread.ownerAppSessionId, 200)) return false;
    if (owners.has(thread.appSessionId)) return false;
    owners.set(thread.appSessionId, thread.ownerAppSessionId);
  }
  if (owners.size && [...owners.values()].filter((owner) => owner === undefined).length !== 1) return false;
  ...
  return Array.isArray(value.uncertainTargets) && value.uncertainTargets.length <= 8 && value.uncertainTargets.every((id: unknown) => typeof id === 'string' && owners.has(id));
}
...
export function isProjectResult(value: Record<string, unknown>): boolean {
  ...
  return value.ok === true && (value.projectId === undefined || text(value.projectId, 200)) && (value.appSessionId === undefined || text(value.appSessionId, 200));
}
