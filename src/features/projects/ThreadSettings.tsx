...
export function ThreadSettings({ value, catalog, disabled, onChange }: {
  ...
  return <div className="my-4 flex flex-wrap items-center gap-2">
    <ProviderPicker value={value.provider} locked={disabled} open={open} onOpenChange={setOpen}
      onSelect={(provider) => onChange({ ...value, provider, modelId: '', reasoning: undefined })} />
    <select aria-label="Thread model" value={value.modelId} disabled={disabled}
      onChange={(event) => onChange({ ...value, modelId: event.target.value, reasoning: undefined })} className={selectClass}>
    ...
    {catalog.efforts.length > 0 && <select aria-label="Thread reasoning" value={value.reasoning ?? ''} disabled={disabled}
      onChange={(event) => onChange({ ...value, reasoning: catalog.efforts.find((effort) => effort === event.target.value) })} className={selectClass}>
      <option value="">Default reasoning</option>
      {catalog.efforts.map((effort) => <option key={effort} value={effort}>{effort}</option>)}
    </select>}
    <AutonomySelector scope="draft" value={value.autonomy} disabled={disabled}
      onSelect={(autonomy) => onChange({ ...value, autonomy })} placement="down" />
  </div>;
}
