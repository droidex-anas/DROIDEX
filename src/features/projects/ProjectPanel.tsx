...
export function ProjectPanel({ project, onOpen, onSpawn }: {
  project: ProjectView;
  ...
  return <>
    <header className="shrink-0 border-b border-droid-border/50 px-6 pb-4 pt-3">
      <div className="flex items-center gap-3">
        ...
        <button type="button" disabled={pending || cannotResume} onClick={() => void togglePaused()} className="rounded-xl border border-droid-border/60 px-3 py-1.5 text-xs text-droid-text-secondary hover:bg-droid-elevated disabled:opacity-40 focus-visible:ring-2 focus-visible:ring-droid-text-muted">{pauseLabel}</button>
      </div>
      ...
      {shownError && <p role="alert" className="mt-3 text-xs leading-5 text-droid-text-secondary">{shownError}</p>}
      {project.uncertain > 0 && <div className="mt-3 rounded-xl border border-droid-border/60 p-3 text-xs text-droid-text-secondary">
        ...
        <div className="my-2 flex flex-wrap gap-2">{project.uncertainTargets.map((id) => <button type="button" key={id} onClick={() => onOpen(id)} className="rounded-lg bg-droid-elevated px-2 py-1">Review {project.threads.find((thread) => thread.appSessionId === id)?.title ?? 'thread'}</button>)}</div>
    ...
    <div className="min-h-0 flex-1 overflow-auto px-6 py-6">
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">{project.threads.map((thread) => <ProjectThreadCard
        key={thread.appSessionId} thread={thread} busy={pending}
        ownerTitle={project.threads.find((item) => item.appSessionId === thread.ownerAppSessionId)?.title}
        disabled={project.paused || pending || project.threads.length + project.launching >= 8}
        onOpen={() => onOpen(thread.appSessionId)} onSpawn={() => onSpawn(thread.appSessionId)} onStop={() => void stop(thread.appSessionId)} />)}</div>
    </div>
  </>;
}
