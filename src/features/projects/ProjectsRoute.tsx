...
export function ProjectsRoute() {
  ...
  const owner = useStoreSelector((state) =>
    form?.ownerId ? state.sessions[form.ownerId] : undefined,
  );
  const cwd = useStoreSelector((state) =>
    state.activeAppSessionId ? (state.sessions[state.activeAppSessionId]?.cwd ?? '') : '',
  );
  ...
  function openThread(appSessionId: string): void {
    // Normal navigation keeps Review, diffs, browser and all session controls intact.
          ...
          {form ? (
            ...
            <div className="mx-auto flex max-w-md flex-col items-center px-6 pt-24 text-center">
              ...
              <h1 className="text-[20px] font-semibold tracking-tight">
                {snapshot.loading ? 'Loading projects…' : 'Projects'}
              </h1>
            ...
            <>
              <header className="shrink-0 border-b border-droid-border/50 px-6 pb-4 pt-3">
                <div className="flex items-center gap-3">
                  ...
                  <button
                    type="button"
                    disabled={pending || (project.paused && project.uncertain > 0 && !reviewed)}
                ...
                <div className="mt-2 flex flex-wrap items-center gap-3 text-[11px] text-droid-text-muted">
                  ...
                  {project.launching > 0 && (
                    <span>
                      Starting {project.launching} thread{project.launching === 1 ? '' : 's'}…
                    </span>
                  )}
                </div>
                {(error || project.error) && (
                ...
                {project.uncertain > 0 && (
                  <div className="mt-3 rounded-xl border border-droid-border/60 p-3 text-xs text-droid-text-secondary">
                    <p>
                      A previous wake may already have reached its conversation. It will not be sent
                      again automatically.
                    </p>
                    <div className="my-2 flex flex-wrap gap-2">
                      {project.uncertainTargets.map((id) => (
                        ...
                        >
                          Review{' '}
                          {project.threads.find((thread) => thread.appSessionId === id)?.title ??
                            'thread'}
                        </button>
                      ))}
                    </div>
                    <label className="flex items-start gap-2">
                      ...
                      />
                      I reviewed these conversations. Discard the uncertain wake without resending
                      it.
                    </label>
                  </div>
                )}
              ...
              <div className="min-h-0 flex-1 overflow-auto px-6 py-6">
                <p className="mb-5 text-[11px] leading-5 text-droid-text-muted">
                  This draft supports the controls below. Agent-native thread tools are not
                  connected yet; no MCP server is added.
                </p>
                <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                  {project.threads.map((thread) => (
                    <ProjectThreadCard
                      ...
                      onOpen={() => {
                        openThread(thread.appSessionId);
                      }}
                      onSpawn={() => {
                        setForm({ ownerId: thread.appSessionId });
                      }}
                      onStop={() => {
                        if (!root) return;
                        setPending(true);
                        setError('');
                        void stopThread(root.appSessionId, thread.appSessionId)
