...
export function ProjectsRoute() {
  ...
  const cwd = useStoreSelector((state) => projectSession(state.sessions, state.activeAppSessionId)?.cwd ?? '');
  const project = snapshot.projects.find((item) => item.id === selectedId) ?? snapshot.projects.at(0);
  ...
  let content: ReactNode = <Empty loading={snapshot.loading} failed={Boolean(snapshot.error)} onCreate={() => setForm({})} />;
  if (project) {
    content = <ProjectPanel key={project.id} project={project} onOpen={openThread} onSpawn={(ownerId) => setForm({ ownerId })} />;
  }
  if (form) {
    content = <div className="overflow-auto px-6 py-8"><NewThreadForm key={form.ownerId ?? 'new'} owner={owner} cwd={cwd} onSubmit={submit} onCancel={() => setForm(null)} /></div>;
  }
    ...
    <div className="flex min-h-0 flex-1">
      <aside aria-label="Projects" className="w-[236px] shrink-0 overflow-auto border-r border-droid-border/60 px-3 py-3">
        <div className="mb-3 flex items-center justify-between px-2">
          ...
          <button type="button" aria-label="New project" disabled={pending} onClick={() => setForm({})} className="rounded-lg p-1.5 text-droid-text-secondary hover:bg-droid-elevated focus-visible:ring-2 focus-visible:ring-droid-text-muted"><Plus size={14} /></button>
        </div>
        <div className="space-y-1">{snapshot.projects.map((item) => <button type="button" key={item.id} disabled={pending} onClick={() => { setSelectedId(item.id); setForm(null); }} aria-current={project?.id === item.id ? 'page' : undefined}
...
function Empty({ loading, failed, onCreate }: { loading: boolean; failed: boolean; onCreate: () => void }) {
  return <div className="mx-auto flex max-w-md flex-col items-center px-6 pt-24 text-center">
    ...
    <h1 className="text-[20px] font-semibold tracking-tight">{loading ? 'Loading projects…' : 'Projects'}</h1>
    ...
    {!loading && !failed && <button type="button" onClick={onCreate} className="mt-5 rounded-xl bg-droid-text px-4 py-2 text-xs font-medium text-droid-bg">Create project</button>}
  </div>;
}
