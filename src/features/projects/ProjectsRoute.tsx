    ...
    <div className="flex min-h-0 flex-1">
      <aside aria-label="Projects" className="w-[236px] shrink-0 overflow-auto border-r border-droid-border/60 px-3 py-3">
        <div className="mb-3 flex items-center justify-between px-2">
          ...
          <button type="button" aria-label="New project" onClick={() => { setForm({}); setError(''); }} className="rounded-lg p-1.5 text-droid-text-secondary hover:bg-droid-elevated focus-visible:ring-2 focus-visible:ring-droid-text-muted"><Plus size={14} /></button>
        </div>
        <div className="space-y-1">{snapshot.projects.map((item) => <button type="button" key={item.id} onClick={() => { setSelectedId(item.id); setForm(null); setError(''); setReviewed(false); }} aria-current={project?.id === item.id ? 'page' : undefined}
