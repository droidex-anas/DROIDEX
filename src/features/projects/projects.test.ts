...
function session(id: string): SessionSummary {
  return {
    appSessionId: id, provider: 'droid', sessionPurpose: 'chat', interactionMode: 'auto',
...
test('validated project graphs and explicitly acknowledged results cross the bridge', () => {
  ...
  assert.ok(wire({ type: 'project.result', requestId: 'request', ok: false, error: 'Disk unavailable' }));
...
test('malformed graphs, negative budgets and foreign uncertain targets are rejected', () => {
  assert.equal(wire({ type: 'projects.snapshot', projects: [{ ...project, threads: [{}] }] }), null);
  assert.equal(wire({ type: 'projects.snapshot', projects: [{ ...project, wakesLeft: -1 }] }), null);
  assert.equal(wire({ type: 'projects.snapshot', projects: [{ ...project, uncertainTargets: ['foreign'] }] }), null);
  ...
  assert.equal(wire({ type: 'projects.snapshot', projects: Array.from({ length: 33 }, () => project) }), null);
});

test('opening a managed thread uses ordinary activation without stopping its siblings', () => {
  const state: AppState = {
    ...initialState, mainView: 'session', activeAppSessionId: 'main',
    sessions: { main: session('main'), worker: session('worker') },
    sessionOrder: ['main', 'worker'], selectedChild: null,
  };
  const next = reducer(state, { type: 'OPEN_PROJECTS', appSessionId: 'worker' });
