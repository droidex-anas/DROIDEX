...
export function ProjectThreadCard({
  ...
  const state = useStoreSelector((store) => {
    ...
    return {
      ...
      needsUser: Boolean(
        store.pendingPermissions[thread.appSessionId] ||
        store.pendingQuestions[thread.appSessionId],
      ),
    };
  }, shallowEqual);
  let status = 'Idle';
  ...
  return (
    ...
    >
      ...
      <button
        ...
        onClick={onOpen}
        className="group flex w-full items-center gap-2 rounded-lg text-left outline-none focus-visible:ring-2 focus-visible:ring-droid-text-muted"
      >
      ...
      <div className="mt-3 flex flex-wrap gap-1.5 text-[10px] text-droid-text-muted">
        {chips.length ? (
          ...
          <span>This saved conversation is unavailable.</span>
        )}
      </div>
