...
export default function NewProject({
  ...
  const selectClass =
    'min-w-0 max-w-[220px] rounded-lg bg-transparent px-2 py-1 text-xs text-droid-text-secondary outline-none focus-visible:ring-2 focus-visible:ring-droid-text-muted';
  return (
    <div className="mx-auto w-full max-w-2xl px-6 py-10">
      <div className="mb-6 flex items-center gap-3">
        <span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-droid-elevated text-droid-text-secondary">
          <GitBranch size={20} />
        </span>
        <div>
          <h1 className="text-xl font-medium tracking-tight text-droid-text">
            One conversation. Several threads.
          </h1>
          <p className="mt-1 text-sm text-droid-text-muted">
            A local workspace for your main agent and the threads it coordinates.
          </p>
        </div>
      </div>
      <form
        onSubmit={(event) => void submit(event)}
        className="rounded-2xl border border-droid-border bg-droid-bg/40 p-4 shadow-sm"
      >
        <input
        ...
        <textarea
          ...
          placeholder="What are we building? Describe the goal, then let the main thread divide the work."
          value={prompt}
          maxLength={8_192}
        ...
        <div className="mt-3 flex flex-wrap items-center gap-1 border-t border-droid-border pt-3">
          ...
          >
            ...
            {models
              .filter((item) => item.id !== defaultModel?.id)
              .map((item) => (
          ...
          {efforts.length > 0 && (
            <select
              aria-label="Main thread reasoning"
              value={reasoning ?? ''}
              disabled={pending}
              onChange={(event) =>
                setReasoning(efforts.find((effort) => effort === event.target.value))
              }
              className={selectClass}
            >
          ...
          <button
            ...
            disabled={pending || !prompt.trim() || Boolean(unavailable)}
            title={unavailable ?? 'Create project'}
        ...
        <label className="mt-4 flex items-center gap-2 text-xs text-droid-text-muted">
          Workspace
          <input
      ...
      {(error || unavailable) && (
        <p role="alert" className="mt-3 text-sm text-droid-text-secondary">
          {error || unavailable}
        </p>
      )}
      <p className="mt-4 text-center text-xs leading-relaxed text-droid-text-muted">
        The watcher runs locally. Agents only receive a new turn when there is work to do.
        <br />
        Threads share the selected folder; ask the main agent to avoid overlapping file edits.
      </p>
    </div>
  );
}
