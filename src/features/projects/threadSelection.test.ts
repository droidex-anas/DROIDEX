...
const selection: ThreadSelection = { provider: 'codex', modelId: '', reasoning: 'high', autonomy: 'low' };
const status: ProviderStatus = {
  provider: 'codex', readiness: 'ready', defaultModelId: 'code',
  models: [{ id: 'code', displayName: 'Code', isCustom: false, supportedReasoningEfforts: ['low', 'high'] }],
};
...
test('draft input follows this harness catalog and includes only a supported reasoning choice', () => {
  ...
  const input = buildThreadInput({ title: ' ', prompt: '  Build the parser\nCheck it. ', workspace: ' /workspace ' }, selection, catalog);
  assert.deepEqual(input, {
    title: 'Build the parser', prompt: 'Build the parser\nCheck it.', provider: 'codex',
    autonomy: 'low', cwd: '/workspace', modelId: 'code', reasoningEffort: 'high',
  });
});

test('a missing model never inherits a different model’s reasoning capabilities', () => {
  const value = { ...selection, modelId: 'custom' };
  const input = buildThreadInput({ title: 'Task', prompt: 'Work', workspace: '' }, value, selectionCatalog(value, [status]));
