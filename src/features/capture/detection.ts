...
export interface RegionCandidate extends CaptureRect { label: string; confidence: number }
interface Raster { width: number; height: number; data: Uint8ClampedArray }
function difference(raster: Raster, x1: number, y1: number, x2: number, y2: number): number {
  ...
  return (Math.abs(raster.data[a] - raster.data[b]) + Math.abs(raster.data[a + 1] - raster.data[b + 1]) + Math.abs(raster.data[a + 2] - raster.data[b + 2])) / 3;
}
function seams(raster: Raster, box: CaptureRect, vertical: boolean): { position: number; strength: number }[] {
  ...
  for (let pos = 10; pos < length - 10; pos += 1) {
    ...
    for (let offset = 2; offset < span - 2; offset += step) {
      ...
      if (delta >= 5) hits += 1;
      contrast += Math.min(delta, 40); samples += 1;
    }
    const coverage = samples ? hits / samples : 0;
    if (coverage >= 0.7 && contrast / samples >= 5) output.push({ position: pos, strength: coverage });
  }
...
export function detectSurfaceRegions(raster: Raster, sourceWidth: number, sourceHeight: number): RegionCandidate[] {
  if (raster.width < 24 || raster.height < 24 || raster.data.length !== raster.width * raster.height * 4) return [];
  ...
  function split(box: CaptureRect, depth: number) {
    ...
    if (!seam) return;
    const first = { ...box, width: useVertical ? seam.position : box.width, height: useVertical ? box.height : seam.position };
    const second = { x: useVertical ? box.x + seam.position : box.x, y: useVertical ? box.y : box.y + seam.position, width: useVertical ? box.width - seam.position : box.width, height: useVertical ? box.height : box.height - seam.position };
    for (const region of [first, second]) {
      if (region.width < 12 || region.height < 12) continue;
      const crop = clampCrop({ x: region.x * scaleX, y: region.y * scaleY, width: region.width * scaleX, height: region.height * scaleY }, sourceWidth, sourceHeight);
      candidates.push({ ...crop, label: depth === 0 ? 'Detected panel' : 'Detected section', confidence: seam.strength });
